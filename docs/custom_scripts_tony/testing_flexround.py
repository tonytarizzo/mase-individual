...
# Make sure FlexRound is registered with MASE
import sys
import os

# Add the parent directory to the path so we can import the temp module
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

# Force register FlexRound with MASE
...
def main():
    ...
    flexround_config = {
        "weight": {
            ...
            "method": "flexround",          # Use our FlexRound method
            "scope": "local",               # Apply locally per layer
            "structured_sparsity": False,   # Don't use structured sparsity
            "alpha": 0.5,                   # FlexRound alpha parameter
            "beta": 0.25,                   # FlexRound beta parameter
            "bit_width": 8,                 # 8-bit quantization
            "frac_width": 4                 # 4 bits for fractional part
        },
        "activation": {
    ...
    mg, _ = passes.prune_transform_pass(mg, pass_args=flexround_config)
    ...
    # Now apply quantization as well (optional)
    ...
    # Try inference with pruned and quantized model
    print("\n===== TESTING INFERENCE =====")
    ...
    try:
        ...
        # Try inference
        with torch.no_grad():
            outputs = mg.model(input_values, attention_mask=attention_mask)
