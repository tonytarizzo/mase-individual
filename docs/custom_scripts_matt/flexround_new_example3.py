...
def main():
    ...
    dataset_name = "nyalpatel/condensed_librispeech_asr"  # or your dataset
    ...
    # Split the model: quantize only the encoder
    encoder = model.wav2vec2  # static portion
    ctc_head = model.lm_head  # keep unquantized
    ...
    # Build a vocabulary and decoder
    ...
    before_params, before_nonzero, _ = print_parameter_count(
        mg.model, "Encoder BEFORE QUANTIZATION"
    )
    
    # 2. Apply FlexRound quantization
    flexround_config = {
        "by": "type",
        "default": {"config": {"name": None}},  # do not alter unlisted ops
        "linear": {
            "config": {
                "name": "flexround",
                "weight_width": 8,
    ...
    after_params, after_nonzero, _ = print_parameter_count(
        mg_quant.model, "Encoder AFTER QUANTIZATION"
    )
    ...
    # 4. Set up Trainer on the small dataset
    tokenized_dataset = DatasetDict({"train": small_dataset, "test": small_dataset})
    ...
    print("Quantization Pass Evaluation:")
